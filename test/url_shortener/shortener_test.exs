defmodule UrlShortener.ShortenerTest do
  ...
  describe "get_url/1" do
    test "return url when given a valid hashed" do
      ...
      fetched_url = Shortener.get_url(shorten_url.hashed)
  ...
  describe "increase_count/1" do
    test "must increase count every time shorten url is called with a valid hashed" do
      ...
      Shortener.increase_count(result.hashed)
      Shortener.increase_count(result.hashed)
      Shortener.increase_count(result.hashed)
    ...
    test "must return the state when calls with an not_found hashed" do
      ...
      :ok = Shortener.increase_count("invalid_hashed")
  ...
  describe "complete flow" do
    test "must work properly" do
      ...
      Shortener.increase_count(shortened_url_one.hashed)
      Shortener.increase_count(shortened_url_one.hashed)

      Shortener.increase_count(shortened_url_two.hashed)
